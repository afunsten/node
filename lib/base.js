...
module.exports.listDirectory = function(userDirectory, dirList = [], filelist = []){
    //list every item in a directory; returns a list of sub lists
    //test: node -e 'require("./lib/base.js").listDirectory("./lib/tests");'
    console.log('listFiles('+JSON.stringify(userDirectory, null, 2)+')');
    const files = fs.readdirSync(userDirectory);
    //console.log(files)
    var i=0
    for (const item of files) {
        //console.log(userDirectory+":"+i);
        if (dirList.indexOf(item) > -1) {
            //Directory already listed
        } else {
            var itemPath=userDirectory+'/'+item
            //console.log(itemPath)
            const fileStat = fs.statSync(itemPath);
            if (fileStat.isDirectory()) {
                dirList.push(itemPath);
                filelist = filelist.splice(i, 1);
                //console.log('directory', path.join(userDirectory, item));
                var mofiles = exports.listDirectory(itemPath, dirList);
                //console.log(JSON.stringify(odir.files, null, 2));
                filelist.push(mofiles);
            } else {
